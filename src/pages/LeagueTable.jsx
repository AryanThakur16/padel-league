      ...
      {summary.some(s => s.matches > 0) && (
        <div>
          ...
          <div className="bg-white border border-slate-200 rounded-xl overflow-hidden">
            <div className="flex justify-between px-4 py-2 bg-slate-50 border-b border-slate-200 text-xs font-semibold text-slate-400 uppercase tracking-wide">
              <div>Session</div>
            ...
            {summary.filter(s => s.players > 0).map(({ session, players: numP, matches: numM, topPlayerId, topPoints }) => (
              ...
              >
                <div>
                  <div className="font-medium text-slate-700 text-sm">Session {session.session_number}</div>
                  <div className="text-xs text-slate-400 mt-0.5">
                    {session.date ? new Date(session.date + 'T00:00:00').toLocaleDateString('en-GB', { day: 'numeric', month: 'short' }) + ' · ' : ''}
                    {numP} players · {numM} matches
                  </div>
                </div>
                <div className="text-right ml-4 flex-shrink-0">
                  {topPlayerId ? (
                    <>
                      <div className="font-medium text-slate-700 text-sm">{playerById[topPlayerId]?.name}</div>
                      <div className="text-xs text-green-700 font-medium">{topPoints} pts</div>
                    </>
